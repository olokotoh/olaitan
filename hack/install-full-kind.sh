...
for f in audit-ca.crt audit-client.crt audit-client.key audit-webhook-values.yaml; do
    [ -e "${cert_dir}/${f}" ] || audit_complete=false
done
...
# into the apiserver.
b64() {
    [ -r "$1" ] || die "cannot read $1 (needed to build the audit kubeconfig)"
    base64 < "$1" | tr -d '\n'
}
...
if [ -e "${out_dir}/calico/calico-values.yaml" ]; then
    info "reusing the Calico material already in ${out_dir}/calico"
else
