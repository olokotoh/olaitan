...
The DFIR agent (`internal/report/dfir/`) is a durable JetStream CONSUMER of the `INCIDENTS` stream (durable `olaitan-dfir-agent`, `AckExplicitPolicy`, `FilterSubject INCIDENTS.finalised`, bounded `MaxDeliver`). On each `IncidentFinalised` event (Story 4.3) it issues a DFIR LLM call through the shared provider abstraction, the model returns a structured `ForensicReport` JSON document, the runner validates it against the embedded `report.v1` schema (the Epic-3 jsonschema pipeline) and then RENDERS it deterministically in Go to a Markdown report with YAML front-matter (the model never controls raw Markdown; this closes the prompt-injection breakout surface). It is OFF by default; enable with `response.dfir.enabled=true`.

**Report sections (FR43).** The rendered Markdown body carries five sections: a chronological kill-chain timeline (FSM transitions + evidence events), MITRE ATT&CK for Containers technique annotations (sourced from the persisted `ThreatAssessment.MitreTechniques`, pinned to ATT&CK for Containers v18), contributing posture findings (RBAC bindings, NetworkPolicies, PodSecurityContext), the containment actions Olaitan took (the non-CLEAN states reached, optional CRIU checkpoint), and a narrative analyst account. The YAML front-matter carries `incident_id`, `attack_techniques[]`, `contributing_posture_findings[]`, `final_fsm_state`, `threat_score_at_decision`, `containment_actions[]`, `report_generated_at`, `prompt_hash`, `dfir_provider`, `dfir_model`. The deterministic front-matter fields are SOURCED from the `IncidentFinalised` event and the persisted assessment (not invented by the model): a model that hallucinates a final state or a technique cannot influence the report header.
...
**Fail-closed degrade.** A provider-unavailable result, a reply truncated by the output-token ceiling (`stop_reason=max_tokens`), or a schema failure after the bounded retry generates NO report, records the failure in `AUDIT.assessments` (status `unavailable`/`schema_violation`), and acks the message: a malformed or truncated report is NEVER persisted. A redelivery of the same finalisation (keyed on `settling.FinalisedMsgID`) does not emit a second report.
