...
**NFR7 budget.** NFR7's 10s p99 is END-TO-END (generation in 4.4 + S3 persist in 4.6); LLM generation dominates the budget. The `olaitan_dfir_report_generation_seconds` histogram observes the agent-invocation-to-rendered-report latency; the DFIR `MaxTokens` is raised to 16000 (the long-form floor in the safe non-streaming band, well under Opus's 128K streaming-only ceiling) so a forensic report is not truncated mid-thought.
...
**Metrics (AC5, NFR7 write tail).** `olaitan_report_writes_total{result}` (counter; `success`/`deduped`/`error`) and `olaitan_report_write_duration_seconds` (histogram) instrument the S3 PUT tail on the same synchronous span as the `olaitan_dfir_report_generation_seconds` generation metric, so the end-to-end generation-to-S3 latency is measurable against NFR7's 10s p99. The metric NAMES align with the Story 4.7 AC4 set (4.7 extends, not renames). The 4.7 deferred-queue gauge (`olaitan_report_writes_deferred_count`) is out of 4.6 scope.
...
**The drain worker (background, `Run(ctx)`).** `internal/report/deferq` runs a background drainer (the `RedactionAuditSink.Run` ticker + wake-on-enqueue + ctx-cancel lifecycle) on a `deferred_drain_seconds` cadence (default 5s). Each pass it PEEKs the head, re-PUTs via the SAME `archive.ReportArchive`, and ONLY after a confirmed (or HEAD-deduped) re-PUT commits the `LPOP` and decrements the gauge (`result="drained"` / `deduped`). A still-transient error STOPS the pass (the head stays, FIFO preserved) until the next tick. A PERMANENT error (or an undecodable envelope) dead-letters the head (loud log, `LPOP`, `result="error"`, gauge decrement) so a poison item never loops forever. CRASH-SAFE + IDEMPOTENT: a crash after the re-PUT but before the `LPOP` re-attempts the SAME content-addressed key on restart, which the 4.6 HEAD-dedup treats as a no-op (no double-write). On startup the gauge is SEEDED from `LLEN reports:deferred` so a restart with a backlog reports the true depth.
...
**Metrics (AC4, NFR28 visibility).** Story 4.7 EXTENDS the 4.6 family (no re-registration): `olaitan_report_writes_total{result}` gains the `retried` / `deferred` / `drained` label values; it ADDS the gauge `olaitan_report_writes_deferred_count` (the live deferred-queue depth, the canonical reconciliation of NFR28's `forensic_writes_deferred_total`: incremented on enqueue, decremented on a drained/dead-lettered head) and the counter `olaitan_report_writes_dropped_total{reason}` (bounded-queue drops). Alert on a sustained non-zero `olaitan_report_writes_deferred_count` (S3 is down) and on ANY `olaitan_report_writes_dropped_total` increment (the backlog exceeded its cap) or `result="error"` write (a permanent misconfiguration or a Redis-down double-outage).
