...
| `olaitan.health.heartbeat`, `olaitan.health.{ring}` | `HealthHeartbeat` (subjects.go:123), `Health` builder (subjects.go:175) | Core NATS, no JetStream stream; ephemeral by design. One live use since Story 10.10: `olaitan.health.applog.<node>` (node-scoped, so a collector hears only its own node's sidecars), published every 30s by each applog sidecar (`internal/collector/applog/heartbeat.go`, `StartHeartbeat`) and consumed by the collector on the same node (`SidecarTracker`); the other rings have no producer or consumer | none (ephemeral) |
