...
| `ConfigOverlay` (`Apply`) | Story 5.3 | FILLED: `helmOverlay` (overlay.go) resolves `--config <name>` to its committed `deploy/helm/olaitan/values-eval-<name>.yaml` overlay, runs `helm upgrade --install --values <overlay> --wait --timeout <budget>`, then an explicit `kubectl rollout status deploy/<release>-olaitan-aggregator` Ready gate (fail-closed). The six arms (F / RS / RSL / RSLT-full / RSLT-L1-only / RSLT-L1+L2) each select the already-shipped chart matrix; the LLM arms pin `analyst.api.model` |
