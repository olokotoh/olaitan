...
var configOnly = []configOnlyParam{
	{
		...
		effect:   "namespaces the response ring never isolates or overrides, and where one Falco alert alone opens no investigation; their events are still correlated and scored, so kube-system stays detected",
		ref:      "FR47",
		location: "config/olaitan.yaml (response.excluded_namespaces)",
	},
	{
		name:     "detection.correlator.never_scored_namespaces",
		typ:      "array",
		def:      "[olaitan]",
		rang:     "list of namespace names",
		effect:   "namespaces whose events are dropped before correlation, so no path scores a workload there (Olaitan's own; correlator.neverScoreReleaseNamespace adds the release namespace)",
		ref:      "FR47",
		location: "config/olaitan.yaml (detection.correlator.never_scored_namespaces)",
	},
	{
